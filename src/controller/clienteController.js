...
endpoint.post("/cliente", async (req, resp) => {
  try {
    ...
    resp
      .status(400)
      .send({ message: "Erro ao adicionar cliente", error: error.message });
    ...
    if (error.message === "Cliente com o mesmo nome e telefone já existe.") {
      ...
      resp
        .status(500)
        .send({
          message: "Erro interno ao adicionar cliente",
          error: error.message,
        });
    }
  }
...
endpoint.put("/cliente/:id", async (req, resp) => {
  try {
    ...
    resp
      .status(400)
      .send({ message: "Erro ao alterar cliente", error: err.message });
  }
});
...
endpoint.delete("/cliente/:id", async (req, resp) => {
  try {
    ...
    resp
      .status(400)
      .send({ message: "Erro ao deletar cliente", error: err.message });
  }
});
...
endpoint.get("/cliente", async (req, resp) => {
  try {
    ...
    resp
      .status(400)
      .send({ message: "Erro ao consultar clientes", error: err.message });
  }
});
